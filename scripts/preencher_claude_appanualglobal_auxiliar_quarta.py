...
/ QUARTA-FEIRA) com EXATAMENTE UMA Ronda (ciclo completo) por execucao --
nunca mais de uma. Respeita qualquer alocacao ja existente na coluna AUXILIAR
(nao sobrescreve) e comeca a proxima Ronda a partir da primeira quarta-feira
futura ainda vazia; para rodar a Ronda seguinte, execute o script de novo.

Irmao de `preencher_claude_appanualglobal_auxiliar_domingo.py` -- mesmo
padrao, mas para D. AUXILIAR/AUXILIAR/QUARTA-FEIRA em vez de DOMINGO. Ao
contrario do irmao QUARTA-FEIRA de D. MINISTROS/MINISTRO, aqui nao ha
compatibilidade de TEMA (aba "Livros" e exclusiva do processo de pregacao) e
nao ha CEIA ALTERNADA (todas as regras de AUXILIAR/QUARTA-FEIRA tem
`ceia_alternada=FALSE` cadastrado -- so o grupo DOMINGO usa isso aqui).

Diferenca pedida pelo Clayton, 2026-09-11 (mesma do irmao DOMINGO): quando o
vencedor de um slot tem "ATRIBUIR AOS RECADOS"=TRUE em BP ALGORITIMO, o
mesmo colaborador tambem e escrito na coluna RECADOS (e "EMAIL RECADOS")
daquela mesma linha/data, alem de AUXILIAR e "EMAIL AUXILIAR".
...
DEPARTAMENTO, FUNCAO, DIA = "D. AUXILIAR", "AUXILIAR", "QUARTA-FEIRA"
...
def main() -> None:
    ...
    # o mesmo colaborador nao pode ser AUXILIAR numa quarta-feira e de novo,
    # poucos dias depois (ou antes), no domingo. Le TODOS os dias da semana
    # diferentes de QUARTA-FEIRA (na pratica, so DOMINGO existe hoje para
    # AUXILIAR).
    ...
    # valor AUXILIAR atual -- preenchido (Ronda ja fechada) ou vazio.
    ...
    for bloco in blocos:
        ...
        if fechada:
            ...
            # minimo cruzado aqui (mesmo motivo do irmao DOMINGO):
